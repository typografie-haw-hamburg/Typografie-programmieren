...
function draw() {
  ...
  //*** PROBLEM 2: BOUNDS *** Warning -> b.bounds(), not possible to get correct bounds, possible line break within textObj
  //*** LÖSUNG PROBLEM 2: Diese Fehlermeldung tritt immer bei Worten auf, die einen Zeilenumbruch ("line break") beinhalten, da kann er natürlich keine sinnvollen bounds messen, weil das Wort über zwei Zeilen geht. Eine Lösung ist, Zeilenumbrüche zunächst auszustellen:
  textFrame.parentStory.hyphenation = false;

  //*** ÜBRIGENS: damit das Script richtig funktioniert, musst du noch mal die Words mit b.words() abrufen, *nachdem* sich die Textgrößen geändert haben, da sich ja da auch die bounds geändert haben, deshalb an dieser stelle einfach words noch mal updaten (siehe folgende Codezeile). Und auch das leading veränderst du am besten erst *nachdem* die Wörter ihre finale Gröpe haben, dann dadurch verschiebt sich ja auch noch mal alles. Also: Erst Wortgrößen anpassen, dann das Leading und dann noch mal words aktualisieren, damit die aktuellen bounds abgespeichert sind.
  var newWords = b.words(textFrame.parentStory);

  for (var i = 0; i < newWords.length; i++) {
    var wordBounds = b.bounds(newWords[i]);
    b.noFill();
    //***ÜBRIGENS 2: Du hattest die ersten beiden Werte bei b.ellipse vertauscht, weshalb die Kreise an den falschen Stellen landeten. So muss es richtig aussehen :) :
    b.ellipse(wordBounds.left, wordBounds.baseline - wordBounds.height/2, frequency, frequency);
  }
